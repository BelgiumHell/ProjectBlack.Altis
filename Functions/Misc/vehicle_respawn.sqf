...
if(isServer) then{
	while{_n == 1} do{
		if((!alive _vehicle) || (!canMove _vehicle)) then { //true if vehicle is not alive or it cannot move
			sleep 0; //change this to what you like, longer will give you a bigger response delay between unit being killed and the vehicle being deleted
			...
			[_vehicle] spawn logistics_DLL_fnc_syncTowing;
			_n = 0; // break out condition
		};
		sleep 10; // sleep for a bit in order to reduce processing calls (increase this to whatever you like, longer gives better performance but also increases response delay)
	};
};